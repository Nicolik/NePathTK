...
if __name__ == '__main__':
    ...
    path_crops_list = []
    path_dataset_list = []
    qu2json_dir = os.path.join(path_to_export, "qu2json-output")
    poly2json_dir = os.path.join(path_to_export, "poly2json-output")
    if os.path.isdir(qu2json_dir):
        path_crops = os.path.join(qu2json_dir, "rois.csv")
        path_dataset = os.path.join(qu2json_dir, "dataset_detectron2.json")
        path_crops_list.append(path_crops)
        path_dataset_list.append(path_dataset)
    elif os.path.isdir(poly2json_dir):
        csvs = [os.path.join(poly2json_dir, f) for f in sorted(os.listdir(poly2json_dir)) if f.endswith(".csv")]
        jsons = [os.path.join(poly2json_dir, f) for f in sorted(os.listdir(poly2json_dir)) if f.endswith(".json")]
        path_crops_list.extend(csvs)
        path_dataset_list.extend(jsons)

    for path_crops, path_dataset in zip(path_crops_list, path_dataset_list):
        ...
        path_to_export_json2exp = os.path.join(path_to_export, "json2exp-output")
        ...
        for idx, row in df_crops.iterrows():
            ...
            if reader_type in [ReaderType.SCN, ReaderType.CZI, ReaderType.OME_TIFF]:
                image_os = BioformatsReader(path_to_wsi)
                print(f"Reader Type: {reader_type}, idx_s: {idx_s}, {image_os.indexes[idx_s]}")
                if reader_type == ReaderType.CZI:
                    idx_s = idx_s - 1
                orig = image_os.read_resolution(image_os.indexes[idx_s], x, y, xsize, ysize, 40)
                image_os.release_bioformat_reader()
            ...
            # cmask = cv2.bitwise_and(orig, orig, mask=mask)
            # cv2.imwrite(cmask_file, cmask)

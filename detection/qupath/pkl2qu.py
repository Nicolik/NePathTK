...
def get_or_create_entry(qp, path_to_wsi, only_existing_entries=False):
    ...
    for qpi in qp.images:
        if image_id == qpi.image_name:
            ...
            entry = qpi
        elif image_id.endswith(".czi") and (image_id + " - Scene #1") == qpi.image_name:
            entry = qpi
    print(f"[get_or_create_entry] after search, entry = {entry}")
    if not only_existing_entries and not entry:
...
def pickle2qu(path_to_wsi, ddict, qupath_project_dir, only_existing_entries=False):
    ...
    with QuPathProject(qupath_project_dir, mode='a+') as qp:
        ...
        for i, poly_coord in enumerate(ddict["Glomerulus"]):
            # print(f"Poly Coord shape: {poly_coord.shape}")

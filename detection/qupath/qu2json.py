...
import shapely
import pandas as pd
from detection.qupath.defs import (MIN_AREA_BBOX_JSON, CATEGORY_ID)
...
if __name__ == '__main__':
    ...
    export_dir = os.path.join(args.export, 'qu2json-output')
        ...
        for i, image in enumerate(qp.images):
            ...
            if image.hierarchy and image.hierarchy.annotations:
                ...
                if os.path.exists(path_to_image):
                    # shape_object_type = 'Glomerulus'
                    for shape_object_type in ['Glomerulus', 'Artery', 'Arteriole']:
                        tile_offset_x = 100
                        tile_offset_y = 100
                        for shape_object in shapes[shape_object_type]:
                            ...
                            if xg.size > 0 and yg.size > 0:
                                ...
                                if area_bbox > MIN_AREA_BBOX_JSON[shape_object_type]:
                                    ...
                                    logging.warning(f"image_id: {tile_id}, found area ({area_bbox})"
                                                    f" lesser than {MIN_AREA_BBOX_JSON[shape_object_type]}!")

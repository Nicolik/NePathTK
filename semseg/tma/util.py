...
def get_segmenter(target, arch="unet", enc_name="resnet34", device="cuda", use_tta=True):
    if target in ['cmc', 'CMC', 'coarse']:
        return get_cortex_medulla_capsule_segmenter(arch, enc_name, device, use_tta=use_tta)
    elif target in ['aag', 'AAG', 'fine']:
        return get_glomerulus_artery_arteriole_segmenter(arch, enc_name, device, use_tta=use_tta)
    else:
        return None

...
from semseg.paths import get_final_model_bin, get_final_model_aag, get_final_model_cmc
...
class MulticlassSegmentation(pl.LightningModule):
    def __init__(self, arch, encoder_name, in_channels, out_classes, **kwargs):
        super().__init__()
        self.model = smp.create_model(
            arch, encoder_name=encoder_name, in_channels=in_channels, classes=out_classes, **kwargs
    ...
    def shared_step(self, batch, stage):
        ...
        pred = torch.argmax(out, 1).unsqueeze(1)
        tp, fp, fn, tn = smp.metrics.get_stats(pred, mask.long(),
                                               mode='multiclass',
...
def get_unet_tissue():
    ...
    enc_name = "resnet34"
    classes = 1
    pl_state_dict = torch.load(get_final_model_bin(arch, enc_name))
    model = BinarySegmentation(arch, enc_name, in_channels=3, out_classes=classes)

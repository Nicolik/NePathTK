...
import subprocess
import sys
from config_semseg import SelectedConfig, SemSegConfigAAG
...
# Tests
test_tile_bbox = False
test_semseg_aag = True
use_mm = True
test_aggr_wsi_gaa_bin_bbox = True
test_qu2json = False
test_json2exp = False

# Config
config = SelectedConfig
semseg = SemSegConfigAAG
...
os.makedirs(aag_aggregator_output, exist_ok=True)
os.makedirs(aag_detector_output, exist_ok=True)
...
if test_semseg_aag:
    ...
    segments_dir = [t.replace('gaa-tiler-output-bbox', 'gaa-detector-output') for t in tiles_dir]
    ...
    for tile_dir, segment_dir in zip(tiles_dir, segments_dir):
        ...
        cmd = [sys.executable, semseg.semseg_aag,
               "--tile", tile_dir,
               "--output", segment_dir,
    ...
    logging.info(f"Skipping run of {semseg.semseg_aag,}!")

if test_aggr_wsi_gaa_bin_bbox:
    ...
    for wsi_name in wsi_names:
        ...
        subprocess.run([sys.executable, PathMESCnn.AGGREGATE_WSI_BIN_BBOX,
                        "--wsi", wsi_name,
                        "--ext", config.ext,
                        "--masks", aag_detector_output,
                        "--wsi-masks", aag_aggregator_output,
                        "--segmentation-type", "aag",
